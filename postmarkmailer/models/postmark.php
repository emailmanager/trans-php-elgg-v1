...
class Emailmanager {
    ...
    function Emailmanager($params = array())
    {
        ...
        elgg_log('Emailmanager Class Initialized','DEBUG');
    ...
    */
    ...
    function send($from_address = null, $from_name = null, $to_address = null, $to_name = null, $subject = null, $message_plain = null, $message_html = null)
	{
        ...
        if (!function_exists('curl_init'))
        {
        	...
        	elgg_log('Emailmanager - PHP was not built with cURL enabled. Rebuild PHP with --with-curl to use cURL.', 'ERROR');            
            return false;       
        }
		...
		if (is_null($this->api_key)) {
			elgg_log("Emailmanager API key is not set!", 'ERROR');            
            return false; 
		}
		...
		$return = curl_exec($ch);
		elgg_log('EMAILMANAGER JSON: ' . $encoded_data . "\nHeaders: \n\t" . implode("\n\t", $headers) . "\nReturn:\n$return", 'DEBUG');
		...
		elgg_log('EMAILMANAGER http code:' . $httpCode, 'DEBUG');
		
		if (intval($httpCode / 100) != 2) {
			$message = json_decode($return)->Message;
			elgg_log('Error while mailing. Emailmanager returned HTTP code ' . $httpCode . ' with message "'.$message.'"', 'ERROR');
			return false;
		}
		return true;
	}
